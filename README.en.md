...
There are no preset package restrictions and no monthly usage threshold. All consumption is billed immediately based on actual usage, and tiered billing is performed based on the user's daily requests. At the same time, free quotas can be obtained through daily sign-ins in the user backend, and these free quotas will not expire.
